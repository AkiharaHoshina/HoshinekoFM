...
- **File Preview**: optional side panel (Settings → Behavior, off by default) squeezed in on the right when a single file is selected — images, audio, videos (mp4/webm/ogg/mkv, with seeking), PDFs (pdf.js, first 5 pages with a "N pages total" notice), archive listings (zip/tar/7z), Markdown (rendered), and text/code (512 KiB cap); a draggable divider resizes the panel (20%–60%, persisted); multi-selection shows a "cannot preview" placeholder; the panel shares the terminal squeeze with the file list.
