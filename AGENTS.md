...
- 已知坑点（写新用例必读）：
  ...
  - **会话总线重启按钮**（设置页，unresponsive 冲突态出现）：`system:restart-session-bus` 依次尝试 `systemctl --user restart dbus-broker.service` / `dbus.service`（30s 超时），成功后经 `registerSystemHandlers(onSessionBusRestarted)` 回调延迟 2.5s 重新注册后端并作废冲突缓存（`resetBackendConflictCache`）；e2e 37 用 PATH 前置假 systemctl 测 IPC 契约，**绝不重启真实总线**。
