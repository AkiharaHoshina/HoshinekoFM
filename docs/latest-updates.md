...
- **固定文件夹拖拽排序（places 栏固定区）**：固定区条目可拖拽
  （HTML5 DnD，仅排序语义）——拖起后**源条目隐藏本体**（只显示跟随
  鼠标的拖拽图像），拖到目标条目时按其**上/下半区**在对应位置生成
  **空占位按钮（虚线边界）**：上半区 = 插到该项前面、下半区 = 插到
  后面（悬停最后一项下半区即放到末尾），松手确认落位。此拖拽仅用于
  调整顺序：位置/设备/添加按钮/文件列表等其他区域一律不接收（落下
  即回弹复原）。
- **防抖与稳定性**：静态列表 + 占位槽模型（列表本身不随拖拽移位，
  反馈只有占位一个元素——相比早期「实时重排」版本彻底消除反馈回路
  振荡）；上/下半区分界 ±4px 死区（光标在中线附近微抖不翻转插入
  位置），drop 时在死区内提交**当前显示的占位间隙**（所见即所得）。
- **拖拽图像时序**：源条目隐藏延迟到 `requestAnimationFrame`——先让
  浏览器截取拖拽图像再隐藏本体（否则截到空白拖影），`ref` 守卫
  「起拖瞬间松手」竞态；源条目仍保留在 DOM（display:none，挂列表
  末尾）保证 ESC 取消时 dragend 正常派发、状态清理。
- **持久化与同步**：顺序经 `sidebar.pinned`（localStorage）持久化，
  复用既有同步通道——主窗口之间 storage 事件实时同步；主窗口
  `app:set-pinned-dirs` 上报快照 → watcher 广播，**打开中的选择器/
  保存器实时跟随新顺序**（含连续多次排序）。选择器/保存器固定区
  **只读**：条目不可拖拽（draggable 关闭）、无移除按钮，不允许改变
  顺序。
- e2e 47（`47-pinned-reorder-sync.test.cjs`）：跨窗口 localStorage 写入
  （等价于另一主窗口完成排序）→ 主窗口实时变序断言 + 打开中的选择器
  实时跟随两次连续排序断言 + 主窗口条目 draggable=true / 选择器
  draggable=false 断言。e2e 25/32/46 回归通过。
...
- **标题栏开关三态逻辑修正（跟随系统 / 开 / 关）**：设置页标题栏开关
  此前直接把 `onClick` 挂在受控 `md-switch` 上——md-switch 内部有独立
  checkbox 状态机，与 React 受控赋值存在时序竞争，跟随系统模式下
  「点两次才生效」。现与主题明暗开关同构：md-switch 纯展示化
  （`pointer-events: none` + 内部 input 移出 Tab 序），交互由外层
  `role=switch` 容器接管（键盘 Enter/Space 显式激活），草稿只经
  **函数式更新**计算——语义：按下「跟随系统」→ 切换为跟随模式
  （null）；手动模式点开关 → 普通取反；**跟随模式点开关 → 退出跟随
...
- **可执行文件打开修复（.AppImage 等）**：带扩展名的可执行映像
  ...
  （双击 + 右键两条链路，脚本落标记文件断言）。
- **固定目录嵌套高亮修复**：A 与嵌套的 A/B 同时固定、打开 B 时，
  A 的图标也变实心——固定项图标此前用 `currentPath.startsWith(path)`
  前缀判定（高亮 pill 已是精确匹配，图标与之不一致）。现与高亮一致
  改为精确匹配（`currentPath === item.path`）：仅当前打开的固定项
  实心；e2e 46 覆盖（A/B 双固定互斥实心断言 + 反向验证）。
- 验证：`npx tsc -b`、`npm run lint` 通过；e2e 03/15/24/27/32/44/45/46 通过。

## v0.11.36 — 搜索分类抑制逻辑修正

- **搜索失败复位**：主窗口与选择器/保存器搜索失败（或后端不可用）时
  不再停留在搜索态——复位搜索状态并重载当前目录，恢复到普通浏览视图。
  此前失败后文件区仍是旧目录内容、右上角分类开关却被搜索态锁定，
  点击无效且无任何反馈。
- **回收站搜索不参与搜索分类**：回收站内搜索是按名称过滤虚拟目录，
  条目所在目录统一为 Trash/files，无按目录分类语义——不再锁定分类
  开关、不再按目录聚簇分组（回落普通语义分组，组头不再误导）。
- **选择器/保存器搜索分类聚簇修复**：`sortedFiles` 补上与主窗口一致的
  `sortFilesByDir` 分支——此前搜索结果按名称/大小/日期全局排序，同
  目录条目被打散、同一目录组头重复出现（分类开关高亮宣称已按目录
  分组，实际显示错误）。
- 主窗口新增派生状态 `searchGroupActive`（搜索中 + 搜索分类开启 +
  非回收站），分类开关强制锁定、按目录聚簇排序、分组渲染三处同源。
- **目录切换滚动回顶**：FileList 在 `currentPath` 变化时 `scrollToRow(0)`——
  react-window 复用滚动容器、条目数变化不自动复位，此前进入新目录
  停留在旧目录滚动位置（主窗口/选择器/保存器同源修复）；同一路径
  刷新（fs 事件/重命名/粘贴等）不触发，保持位置不跳动；效果声明在
  scrollToFileName 之前，定位/揭示流程先回顶再滚到目标行。
...
- **滚动文本/文件预览开关确认时生效**：与语言/完整路径同款 pending 机制——对话框
...
## v0.11.34 — 选择器/保存器 GUI 模式设置回落（标题栏完整路径失效修复）
...
- **选择器搜索升级**（`src/components/FilePicker.tsx`）：与主窗口同款——
  搜索态显示结果计数 + 清除按钮 + 类型（文件/文件夹）/最小/最大大小
  筛选器（同一条 `system:search` IPC，类型选择即重搜、大小输入 Enter
  提交），搜索结果按注入的 `searchGroupByDir` 分组（组头 = 目录路径）。
- **标题栏完整路径同步**（确认时同步组新增 `showFullPathTitle`）：
  设置里启用「标题栏显示完整路径」并确定后，portal 窗口标题显示为
  「选择文件夹：完整目录」/「保存文件：完整目录」等（`pickerTitle` +
  `：` + `currentPath`），关闭时回落仅模式标题；经 `app:set-picker-settings`
  快照 + `picker:settings-changed` 广播，打开中的选择器/保存器实时跟随。
- e2e 41 扩展：settings 注入双字段断言 + 标题完整路径开启/广播回落断言
  + 选择器搜索筛选器与分组头断言。
...
- **现象**：places 栏设备条目（U 盘分区等）胶囊高 48px，与其他胶囊
  条目（固定文件夹、仪表盘等）56px 不一致。
- **修复**（`src/components/Sidebar.css`）：`.sidebar-partition` 高度
  48px → 56px，与 `.sidebar-item` 统一（分区条目虽为两行布局——名称 +
  挂载点副标题，M3 Navigation Drawer 带副文本条目同为 56px）；
  GVfs 卷条目复用同一类，一并统一。
