...
- **"性能优先"**：基于虚拟列表（react-window）的文件列表，支持网格/列表双视图（右上角一键切换）、语义分组与多维排序；图标大小 16–128px 可调（设置滑条或文件区上 Ctrl+滚轮缩放）。
...
- **自动创建启动器条目**：桌面快捷方式与应用程序菜单条目（两个独立开关，设置 → 行为，默认开启，确定时生效）——**开启并确定 → 创建**（首次启动即建；创建一次，手动删除不重建，已存在的条目绝不覆盖）；**关闭并确定 → 删除**（重新开启并确定可再建）。Exec 优先 AppImage 路径、图标复制到 `~/.local/share/icons/hicolor/`；GNOME 需「桌面图标」扩展才能看到桌面快捷方式。
...
- **固定项**：仪表盘可固定文件与文件夹（支持拖拽排序），侧边栏可固定目录（同样支持拖拽排序——按目标条目上/下半区决定插前/插后，拖动中显示空占位边界，其他区域不接收）——支持文件管理器选择、目录右键菜单、拖动文件夹到固定按钮三种方式；侧边栏固定顺序在主窗口间实时同步，打开中的选择器/保存器实时跟随（自身固定区只读）。
- **内置文件选择器**：独立选择器窗口贯穿全应用——可选条目类型声明（文件/文件夹/全部/保存）、文件类型过滤（底部下拉：「所有文件」+ 声明类型，标签由 MIME 描述体系生成）、初始目录、并发实例相互独立；**保存模式重名冲突确认**——保存到已有同名条目时弹出覆盖/自动重命名/手动重命名对话框（自动重命名带「原名 → 新名」预览）；选择器/保存器与主窗口**同步个性化设置**（视图模式/图标/排序/搜索分类/标题栏完整路径/语言，服务模式下主进程从 GUI 快照注入并实时广播，因常驻进程的 userData 与 GUI 隔离），且可在选择器/保存器内**直接 Ctrl+滚轮缩放图标或右上角切换视图**（与主窗口双向同步）；亦可作为 **xdg-desktop-portal FileChooser 后端**——外部程序（GTK/Qt）经标准 portal 接口打开本选择器（OpenFile 含过滤器；SaveFile 保存对话框支持 current-name/accept-label 与重名冲突确认；多文件 SaveFiles 不支持；需安装 portal 配置启用，见 docs/portal-filechooser.md）。
