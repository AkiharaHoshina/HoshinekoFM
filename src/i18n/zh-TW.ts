const zhTW = {
  ...
  'context_menu.pin_sidebar': '釘選至側邊欄',
  'context_menu.unpin_sidebar': '從側邊欄取消釘選',
  ...
  'batch.mode_prefix': '新增前置字',
  'batch.mode_suffix': '新增後置字',
  ...
  'dialog.delete.permanent_confirm': (n: number) => `此操作將永久刪除選取的 ${n} 個項目，無法復原。確定繼續嗎？`,
  ...
  'openrule.user_empty': '尚無使用者設定',
  'openrule.system_empty': '尚無系統設定',
  ...
  'openrule.clear_all_confirm': '確定要清除全部使用者設定嗎？所有檔案類型將回歸系統預設開啟方式。',
  'openrule.overridden': '已被使用者設定覆蓋',
  'openrule.copy_unavailable': '無法複製：找不到對應的應用程式（桌面檔案缺失或無 Exec 行）',
  'openrule.hint': '系統設定為唯讀，需要複製設定到使用者設定以修改。同一種檔案，優先使用使用者設定聲明的開啟方式。',
  ...
  'settings.open_rule_manager_desc': '管理各種檔案類型的預設開啟方式（使用者設定優先於系統設定）',
  'settings.open_rule_manager_enter': '進入',
  'newtab.title': '自訂新增分頁目錄',
  'newtab.hint': '支援絕對路徑（例如 /home/user/Downloads）、~/xxx（家目錄下的路徑）、app://dashboard（開啟儀表板）與 trash:// 或 trash://資料夾名稱（開啟資源回收筒或資源回收筒中的目錄）',
  'newtab.invalid': '無效路徑：請輸入絕對路徑（以 / 開頭）、~/xxx（家目錄）、app://dashboard 或 trash:// 開頭的虛擬路徑',
  ...
  'settings.integration_backend_restart_hint': '舊的 portal/FileManager1 常駐程序已清理；本視窗的後端已立即更新接管，無需重啟應用',
  ...
  'settings.portal_version_mismatch_message': (installed: string, current: string) => `已安裝的 Portal 整合版本為 ${installed}，與目前應用版本 ${current} 不一致。舊版 portal 可能無法正常運作，建議重裝以接管`,
  'settings.portal_version_unknown': '未知',
  'settings.portal_version_restart_hint': '重裝將清理舊版常駐行程，本視窗立即以新版本接管 portal 請求，無需重啟應用',
  ...
  'sidebar.pin_single_folder': '請拖動單個資料夾進行釘選',
  ...
  'device.mounting': (path: string) => `正在掛接 ${path}…`,
  'device.unmounting': (path: string) => `正在卸載 ${path}…`,
  'device.mounted': (device: string, mountpoint: string) => `已掛接 ${device} → ${mountpoint}`,
  'device.unmounted': (device: string) => `已卸載 ${device}`,
  'device.mount_failed': (device: string, error?: string) => `掛接 ${device} 失敗` + (error ? `: ${error}` : ''),
  'device.mount_timeout': (device: string) => `掛接 ${device} 逾時，請稍後重試或檢查裝置`,
  'device.mount_no_device': (device: string) => `${device} 已中斷連線或 USB 位址已變更，請重試`,
  ...
  'device.eject_partitions_mounted': (device: string) => `請先卸載 ${device} 上所有已掛接的分區`,
