const zhCN = {
  ...
  'openrule.title': '打开方式配置管理',
  'openrule.user_section': '用户配置',
  'openrule.system_section': '系统配置',
  'openrule.user_empty': '暂无用户配置',
  'openrule.system_empty': '暂无系统配置',
  'openrule.user_subtitle': ((exec: string) => `打开方式：${exec}`),
  'openrule.edit_title': '编辑',
  'openrule.exec_label': '路径',
  'openrule.clear': '清除配置',
  'openrule.quick_import': '快速导入',
  'openrule.clear_all': '清除全部用户配置',
  'openrule.clear_all_confirm': '确定要清除全部用户配置吗？所有文件类型将回归系统默认打开方式。',
  'openrule.overridden': '已被用户配置覆盖',
  'openrule.copy_unavailable': '无法复制：未找到对应的应用程序（桌面文件缺失或无 Exec 行）',
  'openrule.hint': '系统配置为只读，需要复制配置到用户配置以修改。同一种文件，优先使用用户配置声明的打开方式。',
  ...
  'settings.search_group_by_dir': '搜索结果按所在目录分类',
  ...
  'settings.desktop_entry': '桌面图标',
  'settings.desktop_entry_desc': '开启并确定后创建桌面快捷方式；关闭并确定后删除',
  'settings.app_menu_entry': '应用程序菜单条目',
  'settings.app_menu_entry_desc': '开启并确定后创建应用程序菜单条目；关闭并确定后删除',
  'settings.new_tab_path': '新建标签页目录',
  'settings.new_tab_path_edit': '自定义',
  'settings.open_rule_manager': '打开方式配置管理',
  'settings.open_rule_manager_desc': '管理各文件类型的默认打开方式（用户配置优先于系统配置）',
  'settings.open_rule_manager_enter': '进入',
  'newtab.title': '自定义新标签页目录',
  'newtab.hint': '支持绝对路径（如 /home/user/Downloads）、~/xxx（家目录下的路径）、app://dashboard（打开仪表盘）与 trash:// 或 trash://文件夹名（打开回收站或回收站中的目录）',
  'newtab.invalid': '无效路径：请输入绝对路径（以 / 开头）、~/xxx（家目录）、app://dashboard 或 trash:// 开头的虚拟路径',
  ...
  'settings.integration_backend_restart_hint': '旧的 portal/FileManager1 常驻进程已清理；本窗口的后端已立即更新接管，无需重启应用',
  ...
  'settings.portal_version_mismatch_title': '检测到 Portal 版本不一致',
  'settings.portal_version_mismatch_message': (installed: string, current: string) => `已安装的 Portal 集成版本为 ${installed}，与当前应用版本 ${current} 不一致。旧版 portal 可能无法正常工作，建议重装以接管`,
  'settings.portal_version_unknown': '未知',
  'settings.portal_version_restart_hint': '重装将清理旧版常驻进程，本窗口立即以新版本接管 portal 请求，无需重启应用',
  'settings.portal_version_reinstall': '一键重装',
  'settings.portal_version_reinstalled': 'Portal 集成已重装完成',
