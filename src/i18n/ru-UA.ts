...
const ruUA = {
  ...
  'nya.links': 'Ссылки',
  'nya.link.group': 'Группа обсуждения HoshinekoFM',
  'nya.link.channel': 'Канал Хосинэко',
  ...
  'dialog.conflict.operation_save': 'Сохранить',
  'dialog.conflict.title_save': 'Файл уже существует',
  'dialog.conflict.overwrite': 'Перезаписать (заменить существующий файл)',
  ...
  'openrule.title': 'Настройки открытия файлов',
  'openrule.user_section': 'Пользовательские настройки',
  'openrule.system_section': 'Системные настройки',
  'openrule.user_empty': 'Нет пользовательских настроек',
  'openrule.system_empty': 'Нет системных настроек',
  'openrule.user_subtitle': ((exec: string) => `Открывать с: ${exec}`),
  'openrule.edit_title': 'Изменить',
  'openrule.exec_label': 'Путь',
  'openrule.clear': 'Очистить',
  'openrule.quick_import': 'Быстрый импорт',
  'openrule.clear_all': 'Очистить все пользовательские настройки',
  'openrule.clear_all_confirm': 'Очистить все пользовательские настройки? Все типы файлов вернутся к системным приложениям по умолчанию.',
  'openrule.overridden': 'Переопределено пользовательской настройкой',
  'openrule.copy_unavailable': 'Невозможно скопировать: приложение не установлено или не имеет исполняемой строки Exec',
  'openrule.hint': 'Системные настройки доступны только для чтения — скопируйте их в пользовательские для изменения. Для одного типа файла пользовательская настройка имеет приоритет.',
  ...
  'settings.search_group_by_dir': 'Группировать результаты поиска по каталогам',
  ...
  'settings.desktop_entry': 'Ярлык на рабочем столе',
  'settings.desktop_entry_desc': 'Включите и подтвердите, чтобы создать ярлык на рабочем столе; выключите, чтобы удалить',
  'settings.app_menu_entry': 'Пункт меню приложений',
  'settings.app_menu_entry_desc': 'Включите и подтвердите, чтобы добавить пункт в меню приложений; выключите, чтобы удалить',
  'settings.new_tab_path': 'Каталог новой вкладки',
  'settings.new_tab_path_edit': 'Настроить',
  'settings.open_rule_manager': 'Настройки открытия файлов',
  'settings.open_rule_manager_desc': 'Управление приложениями по умолчанию для типов файлов (пользовательские настройки приоритетнее системных)',
  'settings.open_rule_manager_enter': 'Открыть',
  'newtab.title': 'Настройка каталога новой вкладки',
  'newtab.hint': 'Поддерживаются абсолютные пути (например, /home/user/Downloads), ~/xxx (путь в домашнем каталоге), app://dashboard (открыть панель) и trash:// или trash://имя_папки (открыть корзину или каталог в корзине)',
  'newtab.invalid': 'Недопустимый путь: введите абсолютный путь (начинается с /), ~/xxx (домашний каталог), app://dashboard или виртуальный путь trash://',
  ...
  'settings.integration_backend_restart_hint': 'Старые резидентные процессы portal/FileManager1 остановлены; бэкенд этого окна сразу обновлён и отвечает на запросы — перезапуск не требуется',
  ...
  'settings.portal_version_mismatch_title': 'Обнаружено несовпадение версии Portal',
  'settings.portal_version_mismatch_message': (installed: string, current: string) => `Установленная версия интеграции Portal (${installed}) отличается от текущей версии приложения (${current}). Устаревший portal может работать некорректно — рекомендуется переустановить`,
  'settings.portal_version_unknown': 'неизвестно',
  'settings.portal_version_restart_hint': 'Переустановка остановит старые резидентные процессы, и это окно сразу начнёт отвечать на запросы portal новой версией — перезапуск не требуется',
  'settings.portal_version_reinstall': 'Переустановить сейчас',
  'settings.portal_version_reinstalled': 'Интеграция Portal переустановлена',
  ...
  'sort.collapse': 'Свернуть элементы управления',
  'sort.more': 'Ещё элементы управления',
  'sort.expand': 'Развернуть элементы управления',
  'sort.grouping': 'Группировка',
