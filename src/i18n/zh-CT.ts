const zhCT = {
  ...
  'settings.backend_conflict_outdated': (version: string) => `偵測到舊版後端（版本 ${version}）仍在回應 portal 請求，請解除安裝並重裝系統整合以接管`,
  'settings.backend_conflict_no_version': '偵測到舊版後端仍在回應 portal 請求，請解除安裝並重裝系統整合以接管',
  'settings.backend_conflict_unresponsive': '後端服務無回應（疑似殘留程序佔用匯流排名稱），請重裝系統整合或重新啟動工作階段匯流排',
