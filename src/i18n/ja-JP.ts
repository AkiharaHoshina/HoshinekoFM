const jaJP = {
  ...
  'dialog.conflict.title_save': 'ファイルが既に存在します',
  'dialog.conflict.overwrite': '上書き（既存ファイルを置き換え）',
  ...
  'openrule.title': '開き方の設定管理',
  'openrule.user_section': 'ユーザー設定',
  'openrule.system_section': 'システム設定',
  'openrule.user_empty': 'ユーザー設定はありません',
  'openrule.system_empty': 'システム設定はありません',
  'openrule.user_subtitle': ((exec: string) => `開き方：${exec}`),
  'openrule.edit_title': '編集',
  'openrule.exec_label': 'パス',
  'openrule.clear': '設定をクリア',
  'openrule.quick_import': 'クイックインポート',
  'openrule.clear_all': 'すべてのユーザー設定をクリア',
  'openrule.clear_all_confirm': 'すべてのユーザー設定をクリアしますか？すべてのファイルの種類がシステム既定の開き方に戻ります。',
  'openrule.overridden': 'ユーザー設定により上書きされています',
  'openrule.copy_unavailable': 'コピーできません：対応するアプリケーションが見つかりません（デスクトップファイルの欠落または Exec 行なし）',
  'openrule.hint': 'システム設定は読み取り専用です。変更するにはユーザー設定にコピーしてください。同じファイルの種類では、ユーザー設定の開き方が優先されます。',
  ...
  'settings.search_group_by_dir': '検索結果をディレクトリごとに分類',
  ...
  'settings.desktop_entry': 'デスクトップショートカット',
  'settings.desktop_entry_desc': 'オンにして確定するとデスクトップショートカットを作成し、オフにして確定すると削除します',
  'settings.app_menu_entry': 'アプリケーションメニュー項目',
  'settings.app_menu_entry_desc': 'オンにして確定するとアプリケーションメニュー項目を追加し、オフにして確定すると削除します',
  'settings.new_tab_path': '新しいタブのディレクトリ',
  'settings.new_tab_path_edit': 'カスタマイズ',
  'settings.open_rule_manager': '開き方の設定管理',
  'settings.open_rule_manager_desc': 'ファイルの種類ごとの既定の開き方を管理（ユーザー設定がシステム設定より優先）',
  'settings.open_rule_manager_enter': '開く',
  'newtab.title': '新しいタブのディレクトリをカスタマイズ',
  'newtab.hint': '絶対パス（例: /home/user/Downloads）、~/xxx（ホームディレクトリ配下のパス）、app://dashboard（ダッシュボードを開く）、trash:// または trash://フォルダ名（ゴミ箱またはゴミ箱内のディレクトリを開く）に対応',
  'newtab.invalid': '無効なパス：/ で始まる絶対パス、~/xxx（ホームディレクトリ）、app://dashboard、または trash:// で始まる仮想パスを入力してください',
  ...
  'settings.integration_backend_restart_hint': '古い portal/FileManager1 常駐プロセスを終了しました。このウィンドウのバックエンドはすぐに更新・引き継ぎます（再起動不要）',
  ...
  'settings.session_bus_restart_failed': 'セッションバスの再起動に失敗しました',
  'settings.session_bus_restart_confirm_title': 'セッションバス再起動の確認',
  'settings.session_bus_restart_confirm_message': 'セッションバスを再起動すると、すべてのアプリのセッション D-Bus 接続が切断されます（他アプリが一時的に異常になる可能性）。残留ゾンビ名の解消に使用します。続行しますか？',
  'settings.thumb_cache': 'サムネイルキャッシュ',
  'settings.thumb_cache_info': (n: number, size: string) => `${n} 個のファイル · ${size}`,
  'settings.thumb_cache_empty': 'キャッシュは空です',
  'settings.clear_thumb_cache': 'キャッシュを消去',
  'settings.thumb_cache_cleared': (size: string) => `サムネイルキャッシュを消去しました（${size} 解放）`,
  'settings.clear_thumb_cache_failed': 'サムネイルキャッシュの消去に失敗しました',
  ...
  'settings.portal_version_mismatch_title': 'Portal のバージョン不一致を検出',
  'settings.portal_version_mismatch_message': (installed: string, current: string) => `インストール済み Portal 統合のバージョン（${installed}）が現在のアプリのバージョン（${current}）と一致しません。旧版 portal は正常に動作しない可能性があります。再インストールをおすすめします`,
  'settings.portal_version_unknown': '不明',
  'settings.portal_version_restart_hint': '再インストールで古い常駐プロセスが削除され、このウィンドウがすぐに新版で portal 要求に応答します（再起動不要）',
  'settings.portal_version_reinstall': '今すぐ再インストール',
  'settings.portal_version_reinstalled': 'Portal 統合を再インストールしました',
