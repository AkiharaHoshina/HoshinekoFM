const jaJP = {
  ...
  'settings.backend_conflict_outdated': (version: string) => `旧バージョンのバックエンド（バージョン ${version}）が引き続き portal リクエストに応答しています。システム統合をアンインストールして再インストールしてください`,
  'settings.backend_conflict_no_version': '旧バージョンのバックエンドが引き続き portal リクエストに応答しています。システム統合をアンインストールして再インストールしてください',
  'settings.backend_conflict_unresponsive': 'バックエンドサービスが応答しません（残留プロセスがバス名を占有している可能性）。システム統合を再インストールするか、セッションバスを再起動してください',
